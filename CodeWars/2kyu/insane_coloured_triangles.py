...
def triangle(row):
    ...
    def sumLet(a,b):
        if ord(a) + ord(b) == 164: return 'R' #R + R
        if ord(a) + ord(b) == 142: return 'G' #G + G
        if ord(a) + ord(b) == 132: return 'B' #B + B
        if ord(a) + ord(b) == 153: return 'B' #R + G
        if ord(a) + ord(b) == 148: return 'G' #R + B
        if ord(a) + ord(b) == 137: return 'R' #B + G

    def nextLevel(row):
        if len(row) == 1: return row
        
        newRow = []
        for letA, letB in zip(row, row[1:]):
            newRow.append(sumLet(letA, letB))
        result = "".join(newRow)
